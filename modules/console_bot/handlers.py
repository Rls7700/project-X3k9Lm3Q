...
from ..address_book.models.exceptions import (
    RecordException,
    PhoneException,
    EmailException,
    AddressException,
    ...
    def inner(*args):
        try:
            if func.__name__ in ["add_contact", "change_contact"] and len(args[0]) < 2:
                raise ValueError("Please enter input params properly.")

            if func.__name__ not in ["hello", "show_all_contacts", "dump", "load"] and len(args[0]) < 1:
                raise ValueError("Please enter input params properly.")

            return func(*args)

        except (RecordException, PhoneException, EmailException, AddressException, BirthdayException) as e:
...
def change_contact(args):
    ...
    record = book.find(name)
    if not record:
        raise RecordException(f"Contact not found: {name}")
...
def birthdays(args):
    ...
    for r in b_list:
        print(
            f"Contact {Fore.GREEN}{r['name']}{Fore.RESET} "
            f"with birthday {r['birthday']} will celebrate on "
            f"{Fore.GREEN}{r['congratulation_date']}{Fore.RESET}"
        )
